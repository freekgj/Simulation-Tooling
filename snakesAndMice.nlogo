...
to setup
  ...
  let xMice numMice
  let xSnakes numSnakes
  create-snakes xSnakes [
    set color green
    set xcor random 50
    set ycor random 50
    set size 2
    set target 0
    ask snakes [set miceEaten-# 0]
  ]
...
to update
  (if any? mice-on patch-ahead 0
    [
    ask other mice-here [die]
    ask snakes [set miceEaten-# miceEaten-# + 1]
    show word miceEaten-# " mice eaten"
    ]
   )
  (if any? mice in-radius 6[
    set target one-of other mice in-radius 6
